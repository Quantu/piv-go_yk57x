...
func (yk *YubiKey) ykSetManagementKey(oldKey, newKey []byte, touch bool) error {
	var managementKeyType byte
...
func (yk *YubiKey) SetRetries(managementKey []byte, pin string, pinRetries byte, pukRetries byte) error {
	return yk.ykSetRetries(managementKey, pin, pinRetries, pukRetries)
}

func (yk *YubiKey) ykSetRetries(managementKey []byte, pin string, pinRetries byte, pukRetries byte) error {
	if pinRetries < 1 || pukRetries < 1 {
		return fmt.Errorf("pinRetries and pukRetries must both be greater than zero")
	}

	// NOTE: this action requires the management key AND PIN to be authenticated on the same transaction.
...
func (yk *YubiKey) ykGetProtectedMetadata(pin string) (*Metadata, error) {
	// NOTE: this action requires the PIN to be authenticated on the same transaction.
	if err := yk.ykLogin(pin); err != nil {
